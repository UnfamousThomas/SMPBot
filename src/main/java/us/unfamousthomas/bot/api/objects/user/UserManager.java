package us.unfamousthomas.bot.api.objects.user;
...
import us.unfamousthomas.bot.api.tasks.SaveUsersTaskTimer;
...
public class UserManager extends ListenerAdapter {
    ...
    public void onReady(@NotNull ReadyEvent event) {
        List<User> userList = MongoManager.getInstance().getUserDAO().find().asList();
    ...
    private User saveNewUser(User user) {
        MongoManager.getInstance().getUserDAO().save(user);
        users.put(user.getDiscordId(), user);
    ...
    public void saveAll() {
        ...
        for (Map.Entry entry : users.entrySet()) {
            MongoManager.getInstance().getUserDAO().save((User) entry.getValue());
        }
